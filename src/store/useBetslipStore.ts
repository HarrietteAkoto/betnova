    ...
    (set, get) => ({
      ...
      placeBet: (totalOdds, potentialWin) => {
        ...
        set((st) => ({
          ...
          notification: msg || `✅ Successfully placed ${mode} bet!`
        }));
      ...
      cashOutBet: (betId, cashOutValue) => { set((st) => ({ betHistory: st.betHistory.map(b => b.id === betId ? { ...b, status: 'Cashed Out' as const } : b), notification: `✅ Cashed out!` })); setTimeout(() => set({ notification: null }), 3000); },
      ...
      toggleFavorite: (matchId) => set((st) => ({
        favorites: st.favorites.includes(matchId) ? st.favorites.filter(id => id !== matchId) : [...st.favorites, matchId]
      }))
    }),
